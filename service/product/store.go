...
func (s *Store) CreateProduct(payload types.Product) ( error) {
	_,err := s.db.Exec("INSERT INTO products (name,description,price,stock,category,img_url,is_active) VALUES (?,?,?,?,?,?,?)",payload.Name,payload.Description,payload.Price,payload.Stock,payload.Catagory,payload.ImgUrl,payload.IsActive)
     if err != nil {
		return err
	 }
	 return nil
...
func (s *Store) GetProductByIds(id[]uint) ([]types.Product,error){
  placeholders := strings.Repeat(",?",len(id)-1)
